...
public class SystemCallProvider extends ContentProvider {
    ...
    private boolean initSystem() {
        try {
            Slog.d(TAG, "Initializing CarromRuntime system...");
            CarromRuntimeSystem.getSystem().startup();
            Slog.d(TAG, "CarromRuntime system initialized successfully");
            return true;
        } catch (Exception e) {
            Slog.e(TAG, "Failed to initialize CarromRuntime system", e);
            return false;
        }
    }
