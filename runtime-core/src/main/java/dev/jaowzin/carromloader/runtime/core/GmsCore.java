...
public class GmsCore {
    ...
    private static InstallResult installPackages(Set<String> list, int userId) {
        CarromRuntimeCore carromRuntimeCore = CarromRuntimeCore.get();
        for (String packageName : list) {
            if (carromRuntimeCore.isInstalled(packageName, userId)) {
                continue;
            }
            try {
                CarromRuntimeCore.getContext().getPackageManager().getApplicationInfo(packageName, 0);
            } catch (PackageManager.NameNotFoundException e) {
                
                continue;
            }
            InstallResult installResult = carromRuntimeCore.installPackageAsUser(packageName, userId);
    ...
    private static void uninstallPackages(Set<String> list, int userId) {
        CarromRuntimeCore carromRuntimeCore = CarromRuntimeCore.get();
        for (String packageName : list) {
            carromRuntimeCore.uninstallPackageAsUser(packageName, userId);
        }
    }
    ...
    public static boolean isSupportGms() {
        try {
            CarromRuntimeCore.getPackageManager().getPackageInfo(GMS_PKG, 0);
            return true;
    ...
    public static boolean isInstalledGoogleService(int userId) {
        return CarromRuntimeCore.get().isInstalled(GMS_PKG, userId);
    }
}