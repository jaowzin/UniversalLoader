...
import dev.jaowzin.carromloader.runtime.utils.TrieTree;
...
public class IOCore {
    ...
    public String redirectPath(String path) {
        if (TextUtils.isEmpty(path))
            return path;
        if (path.contains("/universalruntime/")) {
            return path;
        }
    ...
    public void enableRedirect(Context context) {
        ...
        try {
            ApplicationInfo packageInfo = CarromRuntimeCore.getBPackageManager().getApplicationInfo(packageName, PackageManager.GET_META_DATA, CarromRuntimeCore.getUserId());
            int systemUserId = CarromRuntimeCore.getHostUserId();
            ...
            File profilesCurDir = new File(profilesRoot, String.format("cur/%d/%s", CarromRuntimeCore.getUserId(), packageName));
            File profilesRefDir = new File(profilesRoot, String.format("ref/%d/%s", CarromRuntimeCore.getUserId(), packageName));
            FileUtils.mkdirs(profilesCurDir.getAbsolutePath());
            FileUtils.mkdirs(profilesRefDir.getAbsolutePath());
            rule.put(String.format("/data/misc/profiles/cur/%d/%s", CarromRuntimeCore.getUserId(), packageName), profilesCurDir.getAbsolutePath());
            rule.put(String.format("/data/misc/profiles/ref/%d/%s", CarromRuntimeCore.getUserId(), packageName), profilesRefDir.getAbsolutePath());

            if (CarromRuntimeCore.getContext().getExternalCacheDir() != null && context.getExternalCacheDir() != null) {
                File external = BEnvironment.getExternalUserDir(CarromRuntimeCore.getUserId());
            ...
            if (CarromRuntimeCore.get().isHideRoot()) {
                hideRoot(rule);
            }
            proc(rule);
        } catch (Exception e) {
    ...
    private void proc(Map<String, String> rule) {
        int appPid = CarromRuntimeCore.getAppPid();
