...
import dev.jaowzin.carromloader.runtime.R;
...
public class LauncherActivity extends Activity {
    ...
    public static void launch(Intent intent, int userId) {
        try {
            ...
            splash.setClass(CarromRuntimeCore.getContext(), LauncherActivity.class);
            ...
            CarromRuntimeCore.getContext().startActivity(splash);
    ...
    private PackageInfo getPackageInfoWithFallback(String packageName, int userId) {
        try {
            ...
            return CarromRuntimeCore.getBPackageManager().getPackageInfo(packageName, 0, userId);
        } catch (Exception e) {
            Slog.w(TAG, "Failed to get package info for " + packageName + " (attempt 1): " + e.getMessage());
            
            try {
                
                return CarromRuntimeCore.getBPackageManager().getPackageInfo(packageName, 
                ...
                try {
                    ...
                    android.content.pm.ApplicationInfo appInfo = CarromRuntimeCore.getBPackageManager()
    ...
    private void launchAppAsync(final Intent launchIntent, final int userId) {
        new Thread(() -> {
            try {
                ...
                CarromRuntimeCore.getBActivityManager().startActivity(launchIntent, userId);
