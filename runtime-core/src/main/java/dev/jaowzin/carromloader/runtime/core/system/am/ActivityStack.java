package dev.jaowzin.carromloader.runtime.core.system.am;
...
import dev.jaowzin.carromloader.mirror.com.android.internal.BRRstyleable;
...
import dev.jaowzin.carromloader.runtime.core.system.pm.PackageManagerCompat;
import dev.jaowzin.carromloader.runtime.proxy.ProxyActivity;
...
import dev.jaowzin.carromloader.runtime.utils.ComponentUtils;
...
public class ActivityStack {
    ...
    public ActivityStack() {
        mAms = (ActivityManager) CarromRuntimeCore.getContext().getSystemService(Context.ACTIVITY_SERVICE);
    }
    ...
    private int startActivityInNewTaskLocked(int userId, Intent intent, ActivityInfo
        ...
        CarromRuntimeCore.getContext().startActivity(shadow);
        return 0;
    }
    ...
    private int realStartActivityLocked(IInterface appThread, Intent intent, String resolvedType,
        ...
        try {
            ...
            BRIActivityManager.get(BRActivityManagerNative.get().getDefault()).startActivity(appThread, CarromRuntimeCore.getHostPkg(), intent,
    ...
    private Intent getStartStubActivityIntentInner(Intent intent, int vpid,
        ...
        try {
            Resources resources = PackageManagerCompat.getResources(CarromRuntimeCore.getContext(), activityInfo.applicationInfo);
            int id;
            ...
            if (windowIsTranslucent) {
                shadow.setComponent(new ComponentName(CarromRuntimeCore.getHostPkg(), ProxyManifest.TransparentProxyActivity(vpid)));
            } else {
                shadow.setComponent(new ComponentName(CarromRuntimeCore.getHostPkg(), ProxyManifest.getProxyActivity(vpid)));
            }
    ...
    public String getCallingPackage(IBinder token, int userId) {
        synchronized (mTasks) {
            ...
            return CarromRuntimeCore.getHostPkg();
        }
    }
    ...
    public ComponentName getCallingActivity(IBinder token, int userId) {
        synchronized (mTasks) {
            ...
            return new ComponentName(CarromRuntimeCore.getHostPkg(), ProxyActivity.P0.class.getName());
        }
    }
