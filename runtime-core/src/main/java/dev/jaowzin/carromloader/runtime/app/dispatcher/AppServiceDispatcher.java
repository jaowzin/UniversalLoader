package dev.jaowzin.carromloader.runtime.app.dispatcher;
...
import dev.jaowzin.carromloader.runtime.entity.ServiceRecord;
import dev.jaowzin.carromloader.runtime.entity.UnbindRecord;
import dev.jaowzin.carromloader.runtime.proxy.record.ProxyServiceRecord;
...
public class AppServiceDispatcher {
    ...
    private final Handler mHandler = CarromRuntimeCore.get().getHandler();
    ...
    public int onStartCommand(Intent proxyIntent, int flags, int startId) {
        ...
        try {
            ...
            CarromRuntimeCore.getBActivityManager().onStartCommand(proxyIntent, stubRecord.mUserId);
            return i;
        } catch (Throwable e) {
            e.printStackTrace();
        }
    ...
    public boolean onUnbind(Intent proxyIntent) {
        ...
        try {
            UnbindRecord unbindRecord = CarromRuntimeCore.getBActivityManager().onServiceUnbind(proxyIntent, CarromRuntimeCore.getUserId());
            ...
            if (destroy || record.decreaseConnectionCount(intent)) {
                ...
                if (destroy) {
                    service.onDestroy();
                    CarromRuntimeCore.getBActivityManager().onServiceDestroy(proxyIntent, CarromRuntimeCore.getUserId());
        ...
        if (record.getService() != null) {
            ...
            try {
                if (destroy) {
                    ...
                    CarromRuntimeCore.getBActivityManager().onServiceDestroy(intent, CarromRuntimeCore.getUserId());
                    mService.remove(new Intent.FilterComparison(intent));
                }
            } catch (Throwable e) {
                e.printStackTrace();
    ...
    private Service getOrCreateService(ProxyServiceRecord proxyServiceRecord) {
        ...
        Service service = CarromRuntimeCore.currentActivityThread().createService(serviceInfo, token);
        if (service == null)
            return null;
