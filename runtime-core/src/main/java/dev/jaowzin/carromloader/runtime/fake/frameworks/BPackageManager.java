...
import dev.jaowzin.carromloader.runtime.core.system.pm.IBPackageManagerService;
...
import dev.jaowzin.carromloader.runtime.utils.TransactionThrottler;
...
public class BPackageManager extends BlackManager<IBPackageManagerService> {
    ...
    public Intent getLaunchIntentForPackage(String packageName, int userId) {
        ...
        List<ResolveInfo> ris = queryIntentActivities(intentToResolve,
                0,
                intentToResolve.resolveTypeIfNeeded(CarromRuntimeCore.getContext().getContentResolver()),
                userId);
    ...
    private Intent createFallbackLaunchIntent(String packageName) {
        try {
            ...
            Intent intent = CarromRuntimeCore.getContext().getPackageManager().getLaunchIntentForPackage(packageName);
            if (intent != null) {
                intent.setFlags(Intent.FLAG_ACTIVITY_NEW_TASK);
                return intent;
            }
        ...
        try {
            ...
            if (file != null && !file.isEmpty()) {
                try {
                    ...
                    PackageInfo packageInfo = CarromRuntimeCore.getPackageManager().getPackageArchiveInfo(file, 0);
                    if (packageInfo != null) {
                        String packageName = packageInfo.packageName;
                        String hostPackageName = CarromRuntimeCore.getHostPkg();
                        if (packageName.equals(hostPackageName)) {
                            Log.w(TAG, "Attempt to install CarromRuntime app detected and blocked: " + packageName);
    ...
    private boolean isInstalledFallback(String packageName) {
        try {
            ...
            CarromRuntimeCore.getContext().getPackageManager().getPackageInfo(packageName, 0);
            return true;
        } catch (Exception e) {
            ...
            if (packageName != null && (packageName.equals("com.media.bestrecorder.audiorecorder") || 
                                       packageName.startsWith("dev.jaowzin.carromloader.runtime"))) {
