...
public class MethodParameterUtils {
    ...
    public static String replaceFirstAppPkg(Object[] args) {
        ...
        for (int i = 0; i < args.length; i++) {
            if (args[i] instanceof String) {
                ...
                if (CarromRuntimeCore.get().isInstalled(value, CarromRuntimeCore.getUserId())) {
                    args[i] = CarromRuntimeCore.getHostPkg();
                    return value;
                }
            }
        }
    ...
    public static void replaceFirstUid(Object[] args) {
        ...
        for (int i = 0; i < args.length; i++) {
            if (args[i] instanceof Integer) {
                int uid = (int) args[i];
                if (uid == CarromRuntimeCore.getBUid()) {
                    args[i] = CarromRuntimeCore.getHostUid();
                }
            }
        }
    }
    ...
    public static void replaceLastUid(Object[] args) {
        ...
        if (index != -1) {
            ...
            if (uid == CarromRuntimeCore.getBUid()) {
                args[index] = CarromRuntimeCore.getHostUid();
            }
        }
    }
    ...
    public static String replaceLastAppPkg(Object[] args) {
        ...
        if (index != -1) {
            ...
            if (CarromRuntimeCore.get().isInstalled(pkg, CarromRuntimeCore.getUserId())) {
                args[index] = CarromRuntimeCore.getHostPkg();
            }
            return pkg;
        }
        return null;
