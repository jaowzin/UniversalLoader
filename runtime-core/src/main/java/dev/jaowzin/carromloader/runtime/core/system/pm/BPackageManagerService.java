...
import dev.jaowzin.carromloader.runtime.core.system.BProcessManagerService;
import dev.jaowzin.carromloader.runtime.core.system.ISystemService;
import dev.jaowzin.carromloader.runtime.core.system.ProcessRecord;
...
import dev.jaowzin.carromloader.runtime.utils.AbiUtils;
import dev.jaowzin.carromloader.runtime.utils.FileUtils;
import dev.jaowzin.carromloader.runtime.utils.Slog;
import dev.jaowzin.carromloader.runtime.utils.compat.PackageParserCompat;
...
public class BPackageManagerService extends IBPackageManagerService.Stub implements ISystemService {
    ...
    public ApplicationInfo getApplicationInfo(String packageName, int flags, int userId) {
        ...
        if (Objects.equals(packageName, CarromRuntimeCore.getHostPkg())) {
            try {
                return CarromRuntimeCore.getPackageManager().getApplicationInfo(packageName, flags);
    ...
    public List<ResolveInfo> queryIntentServices(
        ...
        final String resolvedType = intent.resolveTypeIfNeeded(CarromRuntimeCore.getContext().getContentResolver());
    ...
    public PackageInfo getPackageInfo(String packageName, int flags, int userId) {
        ...
        if (Objects.equals(packageName, CarromRuntimeCore.getHostPkg())) {
            try {
                return CarromRuntimeCore.getPackageManager().getPackageInfo(packageName, flags);
            } catch (PackageManager.NameNotFoundException e) {
                e.printStackTrace();
            }
            return null;
    ...
    private InstallResult installPackageAsUserLocked(String file, InstallOption option, int userId) {
        ...
        try {
            ...
            if (option.isFlag(InstallOption.FLAG_URI_FILE)) {
                ...
                InputStream inputStream = CarromRuntimeCore.getContext().getContentResolver().openInputStream(Uri.parse(file));
            ...
            PackageInfo packageArchiveInfo = CarromRuntimeCore.getPackageManager().getPackageArchiveInfo(apkFile.getAbsolutePath(), 0);
            ...
            String hostPackageName = CarromRuntimeCore.getHostPkg();
            if (packageName.equals(hostPackageName)) {
                return result.installError("Cannot clone CarromRuntime app from within CarromRuntime. This would create infinite recursion and is not allowed for security reasons.");
            }
            
            
            if (packageName.contains("carromruntime") || packageName.contains("niunaijun") || 
                packageName.contains("vspace") || packageName.contains("virtual")) {
                
                Slog.w(TAG, "Installing potentially CarromRuntime-related app: " + packageName + ". Proceed with caution.");
            }

            boolean support = AbiUtils.isSupport(apkFile);
            if (!support) {
                String msg = packageArchiveInfo.applicationInfo.loadLabel(CarromRuntimeCore.getPackageManager()) + "[" + packageArchiveInfo.packageName + "]";
                return result.installError(packageArchiveInfo.packageName,
                        msg + "\n" + (CarromRuntimeCore.is64Bit() ? "The box does not support 32-bit Application" : "The box does not support 64-bit Application"));
            }
            ...
            if (option.isFlag(InstallOption.FLAG_SYSTEM)) {
                aPackage.applicationInfo = CarromRuntimeCore.getPackageManager().getPackageInfo(aPackage.packageName, 0).applicationInfo;
            }
