...
public class AppJobServiceDispatcher {
    ...
    JobService getJobService(int jobId) {
        synchronized (mJobRecords) {
            ...
            try {
                JobRecord record = CarromRuntimeCore.getBJobManager().queryJobRecord(CarromRuntimeCore.getAppProcessName(), jobId);
                record.mJobService = CarromRuntimeCore.currentActivityThread().createJobService(record.mServiceInfo);
