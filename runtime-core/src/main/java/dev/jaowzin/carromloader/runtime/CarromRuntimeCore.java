...
import dev.jaowzin.carromloader.mirror.android.os.BRUserHandle;
import me.weishu.reflection.Reflection;
import dev.jaowzin.carromloader.runtime.app.BActivityThread;
import dev.jaowzin.carromloader.runtime.app.LauncherActivity;
...
import dev.jaowzin.carromloader.runtime.core.GmsCore;
import dev.jaowzin.carromloader.runtime.core.NativeCore;
import dev.jaowzin.carromloader.runtime.core.env.BEnvironment;
import dev.jaowzin.carromloader.runtime.core.system.DaemonService;
...
import dev.jaowzin.carromloader.runtime.core.system.user.BUserInfo;
...
import dev.jaowzin.carromloader.runtime.fake.frameworks.BJobManager;
import dev.jaowzin.carromloader.runtime.fake.frameworks.BPackageManager;
import dev.jaowzin.carromloader.runtime.fake.frameworks.BStorageManager;
import dev.jaowzin.carromloader.runtime.fake.frameworks.BUserManager;
...
import dev.jaowzin.carromloader.runtime.utils.ShellUtils;
import dev.jaowzin.carromloader.runtime.utils.Slog;
import dev.jaowzin.carromloader.runtime.utils.SimpleCrashFix;
import dev.jaowzin.carromloader.runtime.utils.compat.BuildCompat;
import dev.jaowzin.carromloader.runtime.utils.compat.BundleCompat;

import dev.jaowzin.carromloader.runtime.utils.provider.ProviderCall;
import dev.jaowzin.carromloader.runtime.utils.StackTraceFilter;
import dev.jaowzin.carromloader.runtime.utils.SocialMediaAppCrashPrevention;
import dev.jaowzin.carromloader.runtime.utils.DexCrashPrevention;
import dev.jaowzin.carromloader.runtime.utils.NativeCrashPrevention;
import dev.jaowzin.carromloader.runtime.utils.CrashMonitor;
import dev.jaowzin.carromloader.runtime.utils.StoragePermissionHelper;
import dev.jaowzin.carromloader.runtime.utils.LogSender;



@SuppressLint({"StaticFieldLeak", "NewApi"})
@SuppressWarnings({"unchecked", "deprecation"})
public class CarromRuntimeCore extends ClientConfiguration {
    public static final String TAG = "CarromRuntimeCore";

    private static final CarromRuntimeCore sCarromRuntimeCore = new CarromRuntimeCore();
    ...
    public static CarromRuntimeCore get() {
        return sCarromRuntimeCore;
    }
    ...
    public void setCurrentAppUid(int uid, String packageName) {
        ...
        if (uid != mHostUid && uid > Process.FIRST_APPLICATION_UID && uid < Process.LAST_APPLICATION_UID) {
            ...
            Slog.d("CarromRuntimeCore", "Detected sandboxed environment for " + packageName + " with UID: " + uid);
        }
    }
    ...
    public int resolveUidForOperation(int originalUid, String operation) {
        try {
            ...
            if (mIsSandboxedEnvironment && mCurrentAppUid > 0) {
                Slog.d("CarromRuntimeCore", "Resolving UID for " + operation + ": " + originalUid + " -> " + mCurrentAppUid);
            ...
            Slog.e("CarromRuntimeCore", "Error resolving UID for " + operation + ": " + e.getMessage());
            return originalUid;
        }
    }
    ...
    public void doAttachBaseContext(Context context,
        ...
        if (processName.equals(CarromRuntimeCore.getHostPkg())) {
            mProcessType = ProcessType.Main;
            startLogcat();
        } else if (processName.endsWith(getContext().getString(R.string.carrom_runtime_service_name))) {
            mProcessType = ProcessType.Server;
        } else {
            mProcessType = ProcessType.BAppClient;
        }
        if (CarromRuntimeCore.get().isBlackProcess()) {
            BEnvironment.load();
    ...
    public void doCreate() {
        ...
        try {
            ...
            Slog.d(TAG, "CarromRuntime initialization completed in " + totalTime + "ms");
            
        } catch (Exception e) {
            long totalTime = System.currentTimeMillis() - startTime;
            Slog.e(TAG, "CarromRuntime initialization failed after " + totalTime + "ms", e);
    ...
    public InstallResult installPackageAsUser(String packageName, int userId) {
        try {
            ...
            if (packageName.equals(getHostPkg())) {
                return new InstallResult().installError("Cannot clone CarromRuntime app from within CarromRuntime. This would create infinite recursion and is not allowed for security reasons.");
            }
    ...
    private void initNotificationManager() {
        NotificationManager nm = (NotificationManager) CarromRuntimeCore.getContext().getSystemService(Context.NOTIFICATION_SERVICE);
        String CHANNEL_ONE_ID = CarromRuntimeCore.getContext().getPackageName() + ".carromruntime_core";
        String CHANNEL_ONE_NAME = "carromruntime_core";
        if (BuildCompat.isOreo()) {
    ...
    public void closeCodeInit(){
        try {
            Class entry = Class.forName("dev.jaowzin.carromloader.runtime.closecode.Entry");
    ...
    public static dev.jaowzin.carromloader.runtime.entity.AppConfig getAppConfig() {
        try {
            ...
            new Thread(new Runnable() {
                ...
                public void run() {
                    try {
                        ...
                        Intent vpnIntent = new Intent(getContext(), dev.jaowzin.carromloader.runtime.proxy.ProxyVpnService.class);
    ...
    public static boolean isRunningApplication(String packageName, int userId) {
        ...
        try {
            ...
            ServiceManager.get();
            dev.jaowzin.carromloader.runtime.core.system.am.ActivityStack stack =
                (dev.jaowzin.carromloader.runtime.core.system.am.ActivityStack) ServiceManager.getService(ServiceManager.ACTIVITY_MANAGER);
            if (stack == null) return false;
            java.util.Map<Integer, dev.jaowzin.carromloader.runtime.core.system.am.TaskRecord> tasks =
                    dev.jaowzin.carromloader.runtime.utils.Reflector.with(stack).field("mTasks").get();
            if (tasks == null) return false;
            for (dev.jaowzin.carromloader.runtime.core.system.am.TaskRecord task : tasks.values()) {
                if (task.userId == userId && task.taskAffinity != null && task.taskAffinity.contains(packageName)) {
                    
                    for (dev.jaowzin.carromloader.runtime.core.system.am.ActivityRecord activity : task.activities) {
    ...
    public boolean isCarromRuntimeApp(File apkFile) {
        try {
            ...
            Slog.w(TAG, "Error checking if APK is CarromRuntime app: " + e.getMessage());
        }
        return false;
    }
    
    
    public boolean isCarromRuntimeApp(String packageName) {
            ...
            try {
                ...
                if (error != null) {
                    ...
                    if (getContext() != null) {
                        ...
                        if (nm != null) {
                            String channelId = getContext().getPackageName() + ".carromruntime_core";
                            ...
                            builder.setSmallIcon(android.R.drawable.stat_notify_error)
                                   .setContentTitle("CarromRuntime Log Upload Failed")
                    ...
                    if (getContext() != null) {
                        ...
                        if (nm != null) {
                            ...
                            builder.setSmallIcon(android.R.drawable.stat_sys_upload_done)
                                   .setContentTitle("CarromRuntime Log Upload")
