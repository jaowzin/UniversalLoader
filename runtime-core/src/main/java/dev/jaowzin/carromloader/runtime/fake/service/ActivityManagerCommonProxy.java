...
import dev.jaowzin.carromloader.runtime.fake.provider.FileProviderHandler;
...
import dev.jaowzin.carromloader.runtime.utils.compat.StartActivityCompat;
...
public class ActivityManagerCommonProxy {
        ...
        protected Object hook(Object who, Method method, Object[] args) throws Throwable {
            ...
            if (ComponentUtils.isRequestInstall(intent)) {
                ...
                if (file != null && file.exists()) {
                    try {
                        PackageInfo packageInfo = CarromRuntimeCore.getPackageManager().getPackageArchiveInfo(file.getAbsolutePath(), 0);
                        if (packageInfo != null) {
                            String packageName = packageInfo.packageName;
                            String hostPackageName = CarromRuntimeCore.getHostPkg();
                            if (packageName.equals(hostPackageName)) {
                                Slog.w(TAG, "Blocked attempt to install CarromRuntime app from within CarromRuntime: " + packageName);
                                
                                return 0;
                            }
                        }
                    } catch (Exception e) {
                        Slog.w(TAG, "Could not verify if this is CarromRuntime app: " + e.getMessage());
                    }
                }
                
                if (CarromRuntimeCore.get().requestInstallPackage(file, BActivityThread.getUserId())) {
                    return 0;
                }
            ...
            if (dataString != null && dataString.equals("package:" + BActivityThread.getAppPackageName())) {
                intent.setData(Uri.parse("package:" + CarromRuntimeCore.getHostPkg()));
            }

            ResolveInfo resolveInfo = CarromRuntimeCore.getBPackageManager().resolveActivity(
                    intent,
                    GET_META_DATA,
            ...
            if (resolveInfo == null) {
                ...
                resolveInfo = CarromRuntimeCore.getBPackageManager().resolveActivity(
                        intent,
                        GET_META_DATA,
                        StartActivityCompat.getResolvedType(args),
                        BActivityThread.getUserId());
            ...
            CarromRuntimeCore.getBActivityManager().startActivityAms(BActivityThread.getUserId(),
    ...
    public static class StartActivities extends MethodHook {
        ...
        protected Object hook(Object who, Method method, Object[] args) throws Throwable {
            ...
            return CarromRuntimeCore.getBActivityManager().startActivities(BActivityThread.getUserId(),
    ...
    public static class ActivityResumed extends MethodHook {
        @Override
        protected Object hook(Object who, Method method, Object[] args) throws Throwable {
            CarromRuntimeCore.getBActivityManager().onActivityResumed((IBinder) args[0]);
            return method.invoke(who, args);
        }
    }
    ...
    public static class ActivityDestroyed extends MethodHook {
        @Override
        protected Object hook(Object who, Method method, Object[] args) throws Throwable {
            CarromRuntimeCore.getBActivityManager().onActivityDestroyed((IBinder) args[0]);
            return method.invoke(who, args);
        }
    }
    ...
    public static class FinishActivity extends MethodHook {
        @Override
        protected Object hook(Object who, Method method, Object[] args) throws Throwable {
            CarromRuntimeCore.getBActivityManager().onFinishActivity((IBinder) args[0]);
            return method.invoke(who, args);
        }
    }
    ...
    public static class getCallingPackage extends MethodHook {
        @Override
        protected Object hook(Object who, Method method, Object[] args) throws Throwable {
            return CarromRuntimeCore.getBActivityManager().getCallingPackage((IBinder) args[0], BActivityThread.getUserId());
        }
    }
    ...
    public static class getCallingActivity extends MethodHook {
        @Override
        protected Object hook(Object who, Method method, Object[] args) throws Throwable {
            return CarromRuntimeCore.getBActivityManager().getCallingActivity((IBinder) args[0], BActivityThread.getUserId());
        }
    }
}
