...
import dev.jaowzin.carromloader.mirror.android.app.ActivityThreadActivityClientRecordContext;
import dev.jaowzin.carromloader.mirror.android.app.BRActivityClient;
import dev.jaowzin.carromloader.mirror.android.app.BRActivityClientActivityClientControllerSingleton;
...
import dev.jaowzin.carromloader.mirror.android.app.BRActivityThreadCreateServiceData;
import dev.jaowzin.carromloader.mirror.android.app.BRActivityThreadH;
import dev.jaowzin.carromloader.mirror.android.app.BRIActivityManager;
import dev.jaowzin.carromloader.mirror.android.app.servertransaction.BRClientTransaction;
import dev.jaowzin.carromloader.mirror.android.app.servertransaction.BRLaunchActivityItem;
import dev.jaowzin.carromloader.mirror.android.app.servertransaction.LaunchActivityItemContext;
import dev.jaowzin.carromloader.mirror.android.os.BRHandler;
import dev.jaowzin.carromloader.runtime.CarromRuntimeCore;
import dev.jaowzin.carromloader.runtime.app.BActivityThread;
import dev.jaowzin.carromloader.runtime.fake.hook.IInjectHook;
import dev.jaowzin.carromloader.runtime.proxy.ProxyManifest;
import dev.jaowzin.carromloader.runtime.proxy.record.ProxyActivityRecord;
...
public class HCallbackProxy implements IInjectHook, Handler.Callback {
    ...
    private Handler getH() {
        Object currentActivityThread = CarromRuntimeCore.mainThread();
    ...
    private boolean handleLaunchActivity(Object client) {
        ...
        if (activityInfo != null) {
            if (BActivityThread.getAppConfig() == null) {
                CarromRuntimeCore.getBActivityManager().restartProcess(activityInfo.packageName, activityInfo.processName, stubRecord.mUserId);

                Intent launchIntentForPackage = CarromRuntimeCore.getBPackageManager().getLaunchIntentForPackage(activityInfo.packageName, stubRecord.mUserId);
            ...
            CarromRuntimeCore.getBActivityManager().onActivityCreated(taskId, token, stubRecord.mActivityRecord);
            ...
            if(BuildCompat.isTiramisu()){
                ...
                Object record = BRActivityThread.get(CarromRuntimeCore.mainThread()).getLaunchingActivity(token);
    ...
    private boolean handleCreateService(Object data) {
        if (BActivityThread.getAppConfig() != null) {
            ...
            if (!serviceInfo.name.equals(ProxyManifest.getProxyService(BActivityThread.getAppPid()))
                ...
                CarromRuntimeCore.getBActivityManager().startService(intent, null, false, BActivityThread.getUserId());
                return true;
            }
        }
        return false;
