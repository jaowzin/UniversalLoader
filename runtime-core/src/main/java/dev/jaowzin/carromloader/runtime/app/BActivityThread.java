package dev.jaowzin.carromloader.runtime.app;
...
import dev.jaowzin.carromloader.mirror.android.app.ActivityThreadAppBindDataContext;
import dev.jaowzin.carromloader.mirror.android.app.BRActivity;
import dev.jaowzin.carromloader.mirror.android.app.BRActivityManagerNative;
import dev.jaowzin.carromloader.mirror.android.app.BRActivityThread;
import dev.jaowzin.carromloader.mirror.android.app.BRActivityThreadActivityClientRecord;
import dev.jaowzin.carromloader.mirror.android.app.BRActivityThreadAppBindData;
import dev.jaowzin.carromloader.mirror.android.app.BRActivityThreadNMR1;
import dev.jaowzin.carromloader.mirror.android.app.BRActivityThreadQ;
import dev.jaowzin.carromloader.mirror.android.app.BRContextImpl;
import dev.jaowzin.carromloader.mirror.android.app.BRLoadedApk;
import dev.jaowzin.carromloader.mirror.android.app.BRService;
import dev.jaowzin.carromloader.mirror.android.app.LoadedApk;
import dev.jaowzin.carromloader.mirror.android.content.BRBroadcastReceiver;
import dev.jaowzin.carromloader.mirror.android.content.BRContentProviderClient;
import dev.jaowzin.carromloader.mirror.android.graphics.BRCompatibility;
import dev.jaowzin.carromloader.mirror.android.security.net.config.BRNetworkSecurityConfigProvider;
import dev.jaowzin.carromloader.mirror.com.android.internal.content.BRReferrerIntent;
import dev.jaowzin.carromloader.mirror.dalvik.system.BRVMRuntime;
...
import dev.jaowzin.carromloader.runtime.app.dispatcher.AppServiceDispatcher;
import dev.jaowzin.carromloader.runtime.core.CrashHandler;
import dev.jaowzin.carromloader.runtime.core.IBActivityThread;
import dev.jaowzin.carromloader.runtime.core.IOCore;
import dev.jaowzin.carromloader.runtime.core.NativeCore;
import dev.jaowzin.carromloader.runtime.core.env.VirtualRuntime;
import dev.jaowzin.carromloader.runtime.core.system.user.BUserHandle;
import dev.jaowzin.carromloader.runtime.entity.AppConfig;
import dev.jaowzin.carromloader.runtime.entity.am.ReceiverData;

import dev.jaowzin.carromloader.runtime.fake.delegate.AppInstrumentation;
import dev.jaowzin.carromloader.runtime.fake.delegate.ContentProviderDelegate;

import dev.jaowzin.carromloader.runtime.fake.hook.HookManager;
...
import dev.jaowzin.carromloader.runtime.utils.SafeContextWrapper;
import dev.jaowzin.carromloader.runtime.utils.GlobalContextWrapper;
import dev.jaowzin.carromloader.runtime.utils.Slog;
import dev.jaowzin.carromloader.runtime.utils.compat.ActivityManagerCompat;
import dev.jaowzin.carromloader.runtime.utils.compat.BuildCompat;
import dev.jaowzin.carromloader.runtime.utils.compat.ContextCompat;
import dev.jaowzin.carromloader.runtime.utils.compat.StrictModeCompat;
...
public class BActivityThread extends IBActivityThread.Stub {
    ...
    private final Handler mH = CarromRuntimeCore.get().getHandler();
    ...
    public static int getCallingBUid() {
        return getAppConfig() == null ? CarromRuntimeCore.getHostUid() : getAppConfig().callingBUid;
    }
    ...
    public Service createService(ServiceInfo serviceInfo, IBinder token) {
        ...
        try {
            Context context = CarromRuntimeCore.getContext().createPackageContext(
    ...
    public void bindApplication(final String packageName, final String processName) {
        if (Looper.myLooper() != Looper.getMainLooper()) {
            ...
            CarromRuntimeCore.get().getHandler().post(() -> {
    ...
    private Object createBindApplicationData(String packageName, String processName) {
        try {
            ...
            PackageInfo packageInfo = CarromRuntimeCore.getBPackageManager().getPackageInfo(packageName, PackageManager.GET_PROVIDERS, getUserId());
    ...
    public synchronized void handleBindApplication(String packageName, String processName) {
        ...
        PackageInfo packageInfo = CarromRuntimeCore.getBPackageManager().getPackageInfo(packageName, PackageManager.GET_PROVIDERS, BActivityThread.getUserId());
        ApplicationInfo applicationInfo = packageInfo.applicationInfo;
        ...
        Object boundApplication = BRActivityThread.get(CarromRuntimeCore.mainThread()).mBoundApplication();
        ...
        try {
            ...
            BRActivityThread.get(CarromRuntimeCore.mainThread())._set_mInitialApplication(mInitialApplication);
            ContextCompat.fix((Context) BRActivityThread.get(CarromRuntimeCore.mainThread()).getSystemContext());
    ...
    private android.content.pm.ApplicationInfo getApplicationInfo(Object data) {
        try {
            ...
            String packageName = CarromRuntimeCore.getAppPackageName();
            if (packageName != null) {
                PackageInfo packageInfo = CarromRuntimeCore.getBPackageManager().getPackageInfo(packageName, 0, getUserId());
    ...
    private Context createFallbackContext(String packageName) {
        try {
            Context baseContext = CarromRuntimeCore.getContext();
            if (baseContext == null) {
                Slog.e(TAG, "CarromRuntimeCore.getContext() is null, cannot create fallback context");
                return null;
            }
    ...
    private void setApplication(Application application) {
        try {
            mInitialApplication = application;
            BRActivityThread.get(CarromRuntimeCore.mainThread())._set_mInitialApplication(application);
            ...
            for (ProviderInfo providerInfo : providers) {
                try {
                    if (processName.equals(providerInfo.processName) ||
                        ...
                        installProvider(CarromRuntimeCore.mainThread(), context, providerInfo, null);
                    }
    ...
    public static Context createPackageContext(ApplicationInfo info) {
        try {
            return CarromRuntimeCore.getContext().createPackageContext(info.packageName,
                    Context.CONTEXT_INCLUDE_CODE | Context.CONTEXT_IGNORE_SECURITY);
        } catch (Exception e) {
            e.printStackTrace();
        }
    ...
    public IBinder getActivityThread() {
        return BRActivityThread.get(CarromRuntimeCore.mainThread()).getApplicationThread();
    }
    ...
    public IBinder acquireContentProviderClient(ProviderInfo providerInfo) throws RemoteException {
        ...
        for (String auth : split) {
            ContentProviderClient contentProviderClient = CarromRuntimeCore.getContext()
    ...
    public void finishActivity(final IBinder token) {
        mH.post(() -> {
            Map<IBinder, Object> activities = BRActivityThread.get(CarromRuntimeCore.mainThread()).mActivities();
    ...
    public void handleNewIntent(final IBinder token, final Intent intent) {
        mH.post(() -> {
            ...
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP_MR1) {
                newIntent = BRReferrerIntent.get()._new(intent, CarromRuntimeCore.getHostPkg());
            } else {
                newIntent = intent;
            }
            Object mainThread = CarromRuntimeCore.mainThread();
            if (BRActivityThread.get(CarromRuntimeCore.mainThread())._check_performNewIntents(null, null) != null) {
    ...
    public void scheduleReceiver(ReceiverData data) throws RemoteException {
        ...
        mH.post(() -> {
            ...
            try {
                ...
                CarromRuntimeCore.getBActivityManager().finishBroadcast(data.data);
    ...
    public static Activity getActivityByToken(IBinder token) {
        Map<IBinder, Object> iBinderObjectMap =
                BRActivityThread.get(CarromRuntimeCore.mainThread()).mActivities();
    ...
    private void onBeforeCreateApplication(String packageName, String processName, Context context) {
        for (AppLifecycleCallback appLifecycleCallback : CarromRuntimeCore.get().getAppLifecycleCallbacks()) {
        ...
        try {
            ...
            try {
                ...
                if (validContext == null) {
                    validContext = CarromRuntimeCore.getContext();
                }
            } catch (Exception e) {
