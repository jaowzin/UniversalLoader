...
import dev.jaowzin.carromloader.mirror.android.content.pm.BRApplicationInfoL;
import dev.jaowzin.carromloader.mirror.android.content.pm.BRApplicationInfoN;
import dev.jaowzin.carromloader.mirror.android.content.pm.BRPackageParserSigningDetails;
import dev.jaowzin.carromloader.mirror.android.content.pm.BRSigningInfo;
import dev.jaowzin.carromloader.mirror.android.content.res.BRAssetManager;
...
import dev.jaowzin.carromloader.runtime.utils.ArrayUtils;
...
public class PackageManagerCompat {
    ...
    public static PackageInfo generatePackageInfo(BPackage p, int flags, long firstInstallTime, long lastUpdateTime, BPackageUserState state, int userId) {
        ...
        try {
            base = CarromRuntimeCore.getContext().getPackageManager().getPackageInfo(p.packageName, flags);
    ...
    public static ApplicationInfo generateApplicationInfo(BPackage p, int flags, BPackageUserState state, int userId) {
        ...
        try {
            baseApplication = CarromRuntimeCore.getPackageManager().getApplicationInfo(CarromRuntimeCore.getHostPkg(), flags);
        } catch (Exception e) {
            return null;
        }
        String sourceDir = p.baseCodePath;
        if (p.applicationInfo == null) {
            p.applicationInfo = CarromRuntimeCore.getPackageManager()
